...
if (isset($_GET['id']) && isset($_GET['act']) && $_GET['act'] == 'image') {
    $queryRoomImg = $condb->prepare("SELECT * FROM tbl_room_image WHERE ref_room_id=:id");

    // bindParam
    $queryRoomImg->bindParam(':id', $_GET['id'], PDO::PARAM_INT);
    $queryRoomImg->execute();
    $rsImg = $queryRoomImg->fetchAll();
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-4">
                                            ...
                                            <button type="submit" name="btn" value="upload" class="btn btn-primary">อัพโหลดไฟล์ภาพ</button>
                            ...
                            <tbody>
                                ...
                                foreach ($rsImg as $row) { ?>
                                    <tr>
                                        ...
                                        <td><img src="../assets/product_gallery/<?= $row['room_image']; ?>" width="100px"></td>
                                        <td>../assets/product_gallery/<?= $row['romm_image']; ?></td>
                                        ...
                                        <td>
                                            ...
                                            <form action="" method="post">
                                                ...
                                                <input type="hidden" name="room_image" value="<?= $row['room_image']; ?>">
                                                <button type="submit" name="act" value="deleteImg" class="btn btn-danger"> ลบข้อมูล</button>
                                            </form>
        ...
        for ($i = 0; $i < $total; $i++) {
            ...
            if ($tmpFilePath != "") {
                ...
                if (move_uploaded_file($_FILES['upload']['tmp_name'][$i], $path_copy)) {
                             ...
                             $stmtUpload = $condb->prepare("INSERT INTO tbl_room_image
                                (ref_room_id, room_image)
                                VALUES
                                (:ref_room_id, '$newname')
                            ");
                    $stmtUpload->bindParam(':ref_room_id', $_POST['room_id'], PDO::PARAM_INT);
...
if (isset($_POST['id']) && isset($_POST['no']) && isset($_POST['room_image']) && isset($_POST['act']) && $_POST['act'] == 'deleteImg') {
    ...
    $id = $_POST['id'];
    $no = $_POST['no'];
    $product_image = $_POST['room_image'];


    //sql delete

    $stmtDelProductImg = $condb->prepare('DELETE FROM tbl_room_image WHERE no=:no AND ref_room_id=:id');
    $stmtDelProductImg->bindParam(':id', $id, PDO::PARAM_INT);
    $stmtDelProductImg->bindParam(':no', $no, PDO::PARAM_INT);
    $stmtDelProductImg->execute();  //Del=delete
        ...
        echo '<script>
             setTimeout(function() {
              swal({
                  ...
                  window.location = "product.php?id=' . $id . '&act=image"; //หน้าที่ต้องการให้กระโดดไป
              });
            }, 1000);
        </script>';
        exit();
